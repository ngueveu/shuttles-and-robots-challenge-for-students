...
# - Il y a 8 taches/stations possibles, correspondant aux 8 postes
...
for i in range(6):
    ...
    temps.append(
        [0, 0, 0, 0, 0, 0, 0, 0])  # Initialisation de la matrice temps 6x8 ( car 6 produits et 8 postes (ou taches) )
    tache.append([
...
for ligne in contenu:  # on travail ligne par ligne, donc produit par produit
    ...
    if nom != "\n":  # Si la ligne n'est pas vide, donc si elle contient les informations du cahier des charges
        ...
        tache_list = list(
            tache_list)  # On redéfinie tache_list en tableau de str avec chaque caractère dans une cellule, y compris les espaces
        taille = len(
            tache_list)  # on regade le nombre de cellule de tache_list --> le but va être d'enlever tout les espaces, et de regrouper les destinations dans chaque cellule => y compris si une destination est "12" est pas que ce sauvegarde le "1" dans une cellule et le "2" de l'autre
        j = 0  # compteur
        espace = 1  # indique quand y a un espace
        for i in range(
                taille):  # Dans ce for, on va créer la matrice tache_var qui contient les destinations en enlevant les espaces
            if tache_list[i] != " ":
                if espace == 0:
        ...
        for i in range(taille_dest):
            if tache_var[i] < 1 or tache_var[
                i] > 8:  # erreur de déclaration si la tâche n'est pas comprise entre 1 et 8
                erreur_config = 1
        ...
        if taille_dest > MAX_TACHES:  # erreur s'il y a plus de tâches que de cubes dans V-rep
            erreur_config = 1
            print("ERREUR: mauvaise déclaration dans config, nombre max de destinations par produit est {}".format(
                MAX_TACHES))
        ...
        if erreur_config == 0:
            ...
            for i in range(taille):
                ...
                config[produit_num][-1]['station'] = tache_var[i]
                config[produit_num][-1]['duration'] = dure_var[i]
